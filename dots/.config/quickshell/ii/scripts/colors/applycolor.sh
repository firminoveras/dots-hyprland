...
apply_studio() {
  PLUGIN_DIR=$HOME/.local/share/Google/AndroidStudio2025.2.3/IllogicalTheme
  CONFIG_DIR=$XDG_CONFIG_HOME/Google/AndroidStudio2025.2.3/colors

  apply_color "studio" "illogical.icls" "$CONFIG_DIR/Illogical.icls"
  apply_color "studio" "illogical.theme.json" "$PLUGIN_DIR/illogical.theme.json"
  apply_color "studio" "plugin.xml" "$PLUGIN_DIR/META-INF/plugin.xml"

  cd "$PLUGIN_DIR"
  rm -rf "$PLUGIN_DIR/lib"
  zip -r illogical-theme.jar *

  rm -rf illogical.theme.json
  rm -rf illogical.icls
  rm -rf META-INF

  mkdir -p "$PLUGIN_DIR/lib"
  mv illogical-theme.jar "$PLUGIN_DIR/lib/"
}
...
apply_color "sddm" "theme.conf" "/usr/share/sddm/themes/sdt/theme.conf" &
